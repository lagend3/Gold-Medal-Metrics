...
const countryMostWins = (bestWins, country)=> {
  if(['year', 'discipline', 'sport', 'event'].includes(bestWins)){
    return `SELECT ${bestWins}, COUNT(*) AS count FROM GoldMedal WHERE country = '${country}' GROUP BY ${bestWins} ORDER BY COUNT(*) DESC LIMIT 1;`
  }
    return null;
...
const bestYear = country => {
  return countryMostWins('year', country);
};
...
const bestDiscipline = country => {
  return countryMostWins('discipline', country);
};
...
const bestSport = country => {
  return countryMostWins('sport', country);
};
...
const bestEvent = country => {
  return countryMostWins('event', country);
};

/*
Returns a SQL query string that will find the number of male medalists.
*/
const GenderSort = (genderWins, country)=>{
  if(['Men', 'Women'].includes(genderWins)){
    return `SELECT COUNT(DISTINCT name) FROM GoldMedal WHERE country = '${country}' AND gender = '${genderWins}';`
  }
  return null;
};
const numberMenMedalists = country => {
  return GenderSort('Men', country);
};
...
const numberWomenMedalists = country => {
  return GenderSort('Women', country);
};
